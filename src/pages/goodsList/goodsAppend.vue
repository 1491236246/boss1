<template>
  <div class="goodsAppend global_div">
      <content-top titlename="添加商品"></content-top>
      ...
      <div class="form-wrap flex" v-loading="loading">
              ...
              <div class="select-category flex h-center">
                ...
                <div class="flex-1">
                  ...
                  <ul class="category-list" v-if="childCategoryList.length > 0">
                    <li :class="ruleForm.childId == item.id ? 'active' : ''" @click="getChildCategory(item)" v-for="item in childCategoryList">{{ item.typeName }}</li>
                  </ul>
        ...
        <div v-if="stepActive == 3">
                ...
                <el-form-item label="商品规格: " v-if="propList.length > 0">
                  <div class="prop-wrap">
                    <el-checkbox-group v-model="checkProp" v-for="(item,index) in propList" :key="index">
                      ...
                      <el-checkbox :disabled="!isAdd" @change="handleCheckProp(prop, item.name, $event)" :label="item.name + prop" v-for="prop in item.value">{{ prop }}</el-checkbox>
                    </el-checkbox-group>
                    <div>
            ...
            <div class="flex-1">
              ...
              <div class="flex h-center">
                <div class="upload-btn-wrap">
                  ...
                  <input id="upload" @change="uploadGoodImg" type="file" class="upload-input large" accept="image/jpeg, image/png">
                </div>
                <el-button type="primary" style="float:left;margin-top:20px;" @click="dialogVisible = true">从图片库选择</el-button>
              </div>
              <br>
          ...
          <div class="form-item-3 flex">
            ...
            <div class="flex-1">
              <div class="editor">
                <quill-editor v-model="ruleForm.goodsMobileImg" ref="myQuillEditor" :options="editorOption" class="editor"></quill-editor>
                <el-upload
                    id="uploadEditor"
                    hidden
                    :action="serverUrl"
              ...
              <div class="select-category flex h-center" style="margin-top: 20px">
                ...
                <div class="flex-1">
                  ...
                  <ul class="category-list" v-if="childHeaderList.length > 0">
                    <li :class="ruleForm.navChildId == item.id ? 'active' : ''" @click="getChildHeader(item)" v-for="item in childHeaderList">{{ item.navName }}</li>
                  </ul>
    ...
    uploadFiles(file){
      ...
      checkAlbumImg(data){
        if (this.checkAlbumImgList.indexOf(data.imgUrl) < 0){
          if (this.checkAlbumImgList.length + this.imgList.length >= 5){
            Message.warning("最多添加5张");
            return
          }
          this.checkAlbumImgList.push(data.imgUrl);
        } else {
          this.checkAlbumImgList.splice(this.checkAlbumImgList.indexOf(data.imgUrl), 1);
        }
      },
      addImg(){
      ...
      submitGood(){
        ...
        for (let j = 0; j < this.propSpecList.length; j++){
          ...
          if (!data.goodsSalePrice || !data.goodsStock || !data.stockWarning){
            Message.warning("请填写完整商品规格列表");
            return
          }
          if (data.goodsStock > 99999 || !data.stockWarning > 99999){
            Message.warning("库存最大值为99999");
            return
          }
          if (data.goodsSalePrice < 0){
            Message.warning("规格列表销售价格不能小于0");
            return
          }
          if (data.goodsStock < 0){
            Message.warning("规格列表库存不能小于0");
            return
          }
          if (data.stockWarning < 0){
            Message.warning("规格列表库存预警值不能小于0");
            return
          }
        }
      ...
      submitGoods(){
        if (this.ruleForm.navId && !this.ruleForm.navChildId){
          Message.warning('选了一级类目必须选择二级类目');
          return
        }
