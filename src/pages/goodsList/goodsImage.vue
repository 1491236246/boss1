<template>
  <div class="goodsImage global_div">
    ...
    <div class="center_div">
      <div class="list_head">
        ...
        <div class="div_center" v-show="visibleSearch">
          ...
          <el-input placeholder="相册名称关键字" clearable class="search_input" v-model="name"></el-input>
        </div>
      </div>
    </div>
    <div class="list_content">
      <div class="list_title">
        ...
        <el-button class="el_right" @click="$router.push({name:'addimage'})">添加相册</el-button>
      </div>
      <el-table
        ref="multipleTable"
        :data="tableData3"
      ...
      >
        ...
        <el-table-column prop="firstChar" label="封面">
            <template slot-scope="scope">
                <img :src="scope.row.cover | imgsr" style="width:50px;" alt="">
            </template>
        </el-table-column>
  ...
  methods: {
    deleDates(ids){
        this.$confirm("确认删除吗?").then(() => {
            this.$http.post('merchant_goods_galleries/delete_batch',ids).then(() => {
                this.getList();
    ...
    outGoods(ids){
        this.$router.push({name:'addimage',query:{id:ids}});
    },
    dataSearch(){
